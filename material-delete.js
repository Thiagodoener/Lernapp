...
async function lernappDeleteMany(store,ids){if(!ids.length)return;const db=await lernappOpenDB();await new Promise((resolve,reject)=>{const tx=db.transaction(store,"readwrite");const os=tx.objectStore(store);ids.forEach(id=>os.delete(id));tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);});db.close();await window.LernappSync?.recordDeletion(store,ids).catch(()=>{});}
async function lernappPutMany(store,rows){if(!rows.length)return;const db=await lernappOpenDB();const stamp=new Date().toISOString();await new Promise((resolve,reject)=>{const tx=db.transaction(store,"readwrite");const os=tx.objectStore(store);rows.forEach(row=>os.put({...row,updatedAt:stamp}));tx.oncomplete=resolve;tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error);});db.close();}
...
async function deleteDocumentCascade(documentId){
 ...
 // Lernpläne bleiben erhalten; nur Aufgaben des gelöschten Materials werden entfernt.
 const plans=await lernappAll("plans");
 const changedPlans=[];
 for(const plan of plans){
  const tasks=Array.isArray(plan.tasks)?plan.tasks:[];
  const cleaned=tasks.filter(t=>!goalIds.has(t.goalId)&&!cardIds.has(t.cardId));
  if(cleaned.length!==tasks.length)changedPlans.push({...plan,tasks:cleaned,updatedAt:new Date().toISOString()});
 }

 // Prüfungssitzungen bleiben erhalten; nur betroffene Items werden entfernt.
 const sessions=await lernappAll("examSessions");
 const changedSessions=[];
 for(const session of sessions){
  const items=Array.isArray(session.items)?session.items:[];
  const cleaned=items.filter(i=>!goalIds.has(i.goalId)&&!cardIds.has(i.cardId));
  if(cleaned.length!==items.length)changedSessions.push({...session,items:cleaned,updatedAt:new Date().toISOString()});
 }

 // Exam-Definitionen defensiv bereinigen, falls sie Ziel-IDs enthalten.
 const exams=await lernappAll("exams");
 const changedExams=[];
 for(const exam of exams){
  const ids=Array.isArray(exam.goalIds)?exam.goalIds:[];
  const cleaned=ids.filter(id=>!goalIds.has(id));
  if(cleaned.length!==ids.length)changedExams.push({...exam,goalIds:cleaned,updatedAt:new Date().toISOString()});
 }

 await lernappPutMany("plans",changedPlans);
 await lernappPutMany("examSessions",changedSessions);
 await lernappPutMany("exams",changedExams);
 await lernappDeleteMany("reviews",reviews.map(x=>x.id));
 await lernappDeleteMany("evidence",evidence.map(x=>x.id));
 await lernappDeleteMany("mastery",mastery.map(x=>x.id));
 await lernappDeleteMany("gaps",gaps.map(x=>x.id));
 await lernappDeleteMany("flashcards",cards.map(x=>x.id));
 await lernappDeleteMany("goals",goals.map(x=>x.id));
 await lernappDeleteMany("documents",[documentId]);
